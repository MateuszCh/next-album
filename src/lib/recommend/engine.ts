 ...
 */
async function recommendRediscovery(
  profile: Profile,
 ...
 */
async function recommendDiscovery(
  profile: Profile,
): Promise<AlbumRecommendation | null> {
  ...
  for (let attempt = 0; attempt < 5 && seedPool.length > 0; attempt++) {
    ...
    const fresh = similar.filter(
      (s) => !profile.topArtistNames.has(s.name.toLowerCase()),
    );
    const candidates = fresh.length > 0 ? fresh : similar;
    if (candidates.length === 0) continue;

    const pickedArtist = weightedPick(
      candidates,
      (s) => (Number(s.match) || 0.01) * 100,
    );
...
export async function recommendAlbum(
  ...
  if (profile.topAlbums.length === 0 && profile.topArtists.length === 0) {
    throw new Error(
      "No data in your Last.fm profile — listen to some music first.",
    );
  }
